...
class BrowserSession {
  ...
  async pollLogin() {
    ...
    try {
      ...
      if (this.cookies.length === 3) { this.state = COMPLETE; await this.saveCookies(); this.close(); }
    } catch {}
  ...
  async saveCookies() {
    try {
      await fetch('http://127.0.0.1:8000/admin/jobvision-browser-login/complete', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ session_id: this.id, cookies: this.cookies.map(c => ({ name: c.name, value: c.value, domain: c.domain })) }),
      });
    } catch {}
  }
...
app.listen(9999, () => console.log('Browser agent on port 9999'));