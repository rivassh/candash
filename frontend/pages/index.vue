    ...
    <div class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
      ...
      <form @submit.prevent="handleLogin" class="space-y-4">
        <div>
          ...
          <input v-model="email" type="email" class="input" placeholder="admin@talentmatch.local" required />
        </div>
        <div>
      ...
      <div class="mt-6 p-4 bg-gray-50 rounded-lg text-xs text-gray-500">
        <p class="font-medium mb-1">اطلاعات ورود پیش‌فرض:</p>
        <p>ادمین: admin@talentmatch.local / admin123</p>
        <p>کارشناس HR: hr@talentmatch.local / hr123456</p>
      </div>
    </div>
  </div>
</template>